...
def generate_legal_pdf(result, framework: Framework = "ALL") -> bytes:
    ...
    pdf.cell(0, 5, meta["subtitle"], align="C", new_x="LMARGIN", new_y="NEXT")
    pdf.ln(12)
    ...
    for section_title, section_findings, section_color in sections:
        ...
        for finding in section_findings:
            ...
            pdf.cell(130, 5, finding.title[:75], new_x="LMARGIN", new_y="NEXT")
            pdf.set_x(20)
            ...
            pdf.set_text_color(*_MUTED)
            pdf.multi_cell(170, 4, f"Legal basis: {finding.legal_basis}"[:120])
            pdf.set_x(20)

            # Description
            pdf.set_font("Helvetica", "", 7)
            pdf.set_text_color(*_TEXT)
            pdf.multi_cell(170, 4, finding.description[:200])
            pdf.set_x(20)
            ...
            if finding.recommendation and finding.status != "PASS":
                ...
                pdf.set_font("Helvetica", "", 7)
                pdf.set_text_color(*_TEXT)
                pdf.multi_cell(170, 4, finding.recommendation[:200])
                pdf.set_x(20)
