...
<div style="background:#0d1117;border:1px solid #1f2d3d;border-radius:14px;padding:26px 24px;
  ...
  <div style="display:flex;align-items:center;gap:24px;flex-wrap:wrap">
    ...
    for _tn, _tr in _preview:
        ...
        _find   = _html.escape((_tr.get("finding") or "No issues detected.")[:220])
        st.markdown(f"""
...
if pt_mode_active:
    _mode_badge_html = '<span class="cs-badge" style="margin-left:6px;border-color:#ef4444;color:#ef4444">🔴 ACTIVE PT MODE</span>'
elif scan_mode == "passive":
    _mode_badge_html = '<span class="cs-badge" style="margin-left:6px;border-color:#60a5fa;color:#60a5fa">🔵 PASSIVE RECON</span>'
else:
  ...
  {_upgrade_row}
        ...
        def _build_html_report(pr_data: dict, target: str, score: int, grade: str) -> str:
            ...
            for _tn, _tr in sorted(_tools.items(), key=lambda kv: _so.get(kv[1].get("severity","INFO"), 4)):
                ...
                _fi  = _html.escape(_tr.get("finding") or "No issues detected.")
...
<div class="bb-card">
  ...
  {"".join(f'<div class="bb-card-item">🔗 {u}</div>' for u in _bb_urls[:2])}
        ...
        for tool_key, (icon, label) in _sorted_tools:
            ...
            if _raw_finding:
                finding = _html.escape(_raw_finding)
            elif status == "completed":
                finding = t("res_no_issues")
            elif status in ("timeout", "error"):
                finding = f"⚠️ {_html.escape(tr.get('error', 'Service temporarily unavailable — try re-running the scan.'))}"
...
""")
        ...
        # ── Responsible Disclosure email template ──────────────────────────
        if _pr_crits:
            st.markdown(f'<div class="pr-section-title">{t("disclosure_email_tpl")}</div>',
                        unsafe_allow_html=True)
            st.info(
...
with tab_our_legal:
    from legal_pages import show_legal_pages
    show_legal_pages()

# ═════════════════════════════════════════════════════════════════════════════

with tab_code:
    st.markdown(f'<div class="section-label">{t("tab_code_scanner")}</div>', unsafe_allow_html=True)
    ...
    if not _crewai_available:
        st.markdown("""
<div style="background:linear-gradient(135deg,#1a1f2e,#0f1623);border:1px solid #334155;
            border-left:4px solid #f59e0b;border-radius:10px;padding:24px 28px;margin:20px 0">
  <div style="display:flex;align-items:center;gap:12px;margin-bottom:10px">
    <span style="font-size:1.6rem">🔧</span>
    <span style="color:#f59e0b;font-weight:800;font-size:1rem;letter-spacing:.04em;
                 font-family:'JetBrains Mono',monospace">UPGRADING — BACK SOON</span>
  </div>
    ...
    if not _urls2:
        ...
        if diff_url:
            ...
            if len(_hist2) < 2:
                ...
                mc1.metric(t("diff_score_a"), f"{scan_a.overall_score}/100 ({scan_a.overall_grade})")
                mc2.metric(t("diff_score_b"), f"{scan_b.overall_score}/100 ({scan_b.overall_grade})")
                mc3.metric(t("diff_delta"), f"{delta_sign}{delta_score} pts",
                           delta=delta_score, delta_color="normal")

                # ── Per-category deltas ───────────────────────────────────────
                st.markdown(f'<div class="section-label" style="margin-top:16px">{t("diff_category_deltas")}</div>',
                            unsafe_allow_html=True)
...
<div class="score-card">
  ...
  <div class="score-bar-bg"><div class="score-bar-{bar_cls}" style="width:{val_b}%"></div></div>
                ...
                st.markdown(f'<div class="section-label" style="margin-top:16px">{t("diff_findings_delta")}</div>',
                            unsafe_allow_html=True)
                fd_c1, fd_c2, fd_c3 = st.columns(3)
                fd_c1.metric(t("diff_new_findings").strip("*"),  len(new_findings),      delta=len(new_findings) or None,
                             delta_color="inverse")
                fd_c2.metric(t("diff_resolved").strip("*"),      len(resolved_findings), delta=len(resolved_findings) or None,
                             delta_color="normal")
                fd_c3.metric(t("diff_delta"),                    len(unchanged))

                if new_findings:
                    st.markdown(t("diff_new_findings"))
                    for f in new_findings:
                        st.markdown(f'<div class="verify-row verify-confirmed">🔴 NEW: {f}</div>',
                                    unsafe_allow_html=True)
                if resolved_findings:
                    st.markdown(t("diff_resolved"))
